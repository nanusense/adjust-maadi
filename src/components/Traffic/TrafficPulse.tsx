...
const STATUS_CONFIG = {
  severe:   { label: "Jammed",   dot: "#EF4444", bg: "rgba(239,68,68,0.08)",   border: "rgba(239,68,68,0.2)"  },
  heavy:    { label: "Heavy",    dot: "#F97316", bg: "rgba(249,115,22,0.08)",  border: "rgba(249,115,22,0.2)" },
  moderate: { label: "Moderate", dot: "#F59E0B", bg: "rgba(245,158,11,0.08)",  border: "rgba(245,158,11,0.2)" },
  clear:    { label: "Clear",    dot: "#16a34a", bg: "rgba(22,163,74,0.08)",   border: "rgba(22,163,74,0.2)"  },
  // fallback keys from old static logic
    ...
    >
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          >
            ...
            <div
              ...
              style={{
                color: ["Peak Chaos", "Evening Rush"].includes(currentTip.label)
                  ? "#EF4444"
                  : currentTip.label === "Easing Off" ? "#F97316" : "#16a34a",
              }}
            >
              {currentTip.label}
            </div>
      ...
      <div className="divide-y" style={{ borderColor: "rgba(45,80,22,0.06)" }}>
        {TRAFFIC_CORRIDORS.map((corridor) => {
          ...
          const severityKey = liveData
            ? liveData.liveSeverity
            : staticSeverity(corridor, hour);
          const cfg       = STATUS_CONFIG[severityKey] ?? STATUS_CONFIG.clear;
          const isSelected = selected?.name === corridor.name;
          const pulse      = severityKey !== "clear";
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-2.5 min-w-0">
                    <span
                      className={`flex-shrink-0 w-2.5 h-2.5 rounded-full ${pulse ? "animate-pulse" : ""}`}
